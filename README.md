...
<code>add system cmdPolicy geoip2adc_cmdpol ALLOW "((^(scp).\*/var/netscaler/inbuilt_db\*)|(^save\\s+ns\\s+config)|(^save\\s+ns\\s+config\\s+.\*)|(^add\\s+(locationFile|locationFile6)\\s+/var/netscaler/inbuilt_db/Citrix_Netscaler_InBuilt_GeoIP_DB_IPv\*.\*))"</code></br>
